...
def cpma_pipeline(input_folder):
    ...
    values_cmd = f'python get_values.py -i {eqtl_file} -n {num_genes} -p {pvalue_file} -z {zscore_file}'.split(' ')
    ...
    cpma_cmd = f'python calculate_cpma.py -i {pvalue_file} -o {cpma_file}'.split(' ')
...
def iterate_folders(folder, iterations):
    for i in range(iterations):
        ...
        print(f'Starting CPMA for Simulation {i}')
        cpma_pipeline(input_folder)

...
def get_noise(num_genes, cov, sample_size, identity, seed, fast):
    if identity:
        if fast: 
            print('Simulating noise (identity, fast with cholesky)')
            np.random.seed(seed)
            rng = np.random.default_rng(seed)
            return rng.multivariate_normal(np.zeros(num_genes), cov, size=sample_size, method='cholesky')
        else:
...
def iter_model(config, seed, iterations, output, fast):
    print(f'Seed = {seed}')
    np.random.seed(seed)
    ...
    for i in range(iterations):
        ...
        noise_iter = get_noise(num_genes=num_genes, cov=cov, sample_size=sample_size, identity=identity, seed=seed, fast=fast)
...
def main():
    ...
    parser.add_argument("-f", "--fast", action='store_true', help="Fast simulation of noise with cholesky for identity cov matrix")

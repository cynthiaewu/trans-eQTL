...
def cpma_pipeline(input_folder, output):
    ...
    for i in range(1, 23):
        ...
        values_cmd = f'python get_values.py -i {input_folder}/chr{i}_matrixeQTL_raw.tsv -p {output}/chr{i}/{pvalue_file}_chr{i} -z {output}/chr{i}/{zscore_file}_chr{i}'.split(' ')

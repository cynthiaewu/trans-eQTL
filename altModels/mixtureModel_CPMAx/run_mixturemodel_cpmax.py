...
def mixture_pipeline(input_folder, scripts_folder):
    ...
    for tar in targets:
        ...
        for beta in beta_values:
            ...
            mixture_cmd.append(f'python {scripts_folder}/mixtureModel_CPMAx/calculate_mixturemodel_cpmax_iter.py -f {input_folder}/numTarget_{tar}/Beta_{value} -p {scripts_folder} -i 100'.split(' '))
    ...
    for tar in targets:
        ...
        for beta in beta_values:
            value = str(beta).replace(".","")
            power_cmd.append(f'python {scripts_folder}/Simulator/calculate_power_singleqtl_cpma.py -c {input_folder}/numTarget_{tar}/Beta_{value}/metaconfig.yaml -m 6 -x 1.0 -f {input_folder}/numTarget_{tar}/Beta_{value} -i 100'.split(' '))

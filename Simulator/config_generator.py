...
# not currently used, real cov matrix from nerve tibial tissue used instead
...
def generate_beta(num_genes, targets, num_snps, num_nullsnps, beta_type, fixed_betas, mix):    
    all_betas = []
...
def generator(num_genes, num_targets, identity, num_snps, num_nullsnps, beta, beta_sd, beta_value, sim_path, output, mix):
    ...
    if beta == 'value' and not mix:
        if not os.path.isfile(f'{output}/beta.txt'):
            ...
            # same beta for all simulations saved in outer folder
...
def main():
    ...
    parser.add_argument("-m", "--mix", action='store_true', help="If mixing/shuffling the target genes is needed")

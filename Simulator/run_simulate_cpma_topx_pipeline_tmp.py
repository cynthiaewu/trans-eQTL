...
def sim_cpmax_pipeline(input_folder, scripts_folder, topx, samplesize):
    ...
    for tar in targets:
        ...
        for beta in beta_values:
            value = str(beta).replace(".","")
            cpma_cmd.append(f'python {scripts_folder}/CPMA/run_cpmax_pipeline_sim_tmp.py -f {input_folder}/numTarget_{tar}/Beta_{value} -p {scripts_folder} -x {topx} -i 100'.split(' '))

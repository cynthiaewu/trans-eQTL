...
def sim_cpmax_pipeline(input_folder, scripts_folder, topx, samplesize):
    ...
    for tar in targets:
        ...
        for beta in beta_values:
            ...
            configen_cmd.append(f'python {scripts_folder}/Simulator/config_generator_specific.py -c {input_folder}/numTarget_{tar}/Beta_{value}/metaconfig.yaml -i 100 -o {input_folder}/numTarget_{tar}/Beta_{value} -m True'.split(' '))
    ...
    for tar in targets:
        ...
        for beta in beta_values:
            value = str(beta).replace(".","")
            simulate_cmd.append(f'python {scripts_folder}/Simulator/simulate_expression_givenoise_cholesky_chunk.py -c {input_folder}/numTarget_{tar}/Beta_{value}/metaconfig.yaml -i 1 -o {input_folder}/numTarget_{tar}/Beta_{value} -m True'.split(' '))
    ...
    for tar in targets:
        ...
        for beta in beta_values:
            value = str(beta).replace(".","")
            simulate_cmd = f'python {scripts_folder}/Simulator/simulate_expression_givenoise_test.py -c {input_folder}/numTarget_{tar}/Beta_{value}/metaconfig.yaml -i 100 -o {input_folder}/numTarget_{tar}/Beta_{value}'.split(' ')
            subprocess.call(simulate_cmd)
    print('Finished simulating files')

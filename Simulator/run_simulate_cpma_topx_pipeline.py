...
def sim_cpmax_pipeline(input_folder, scripts_folder, topx, samplesize):
    ...
    #targets = [5, 10, 15, 30]
    targets = [300, 400, 700, 1000, 5000, 10000, 15000]
    #targets = [10000]
    beta_values = [0, 0.05, 0.1, 1]
    ...
    for tar in targets:
        ...
        for beta in beta_values:
            value = str(beta).replace(".","")
            cpma_cmd.append(f'python {scripts_folder}/CPMA/run_cpmax_pipeline_sim_performdecomp.py -f {input_folder}/numTarget_{tar}/Beta_{value} -p {scripts_folder} -x {topx} -i 100'.split(' '))

...
def sim_cpmax_pipeline(input_folder, scripts_folder, topx, samplesize):
    ...
    for tar in targets:
        ...
        for beta in beta_values:
            value = str(beta).replace(".","")
            power_cmd.append(f'python {scripts_folder}/Simulator/calculate_power_singleqtl_cpma.py -c {input_folder}/numTarget_{tar}/Beta_{value}/metaconfig.yaml -m 11 -x {topx} -f {input_folder}/numTarget_{tar}/Beta_{value} -i 100'.split(' '))

...
def cpma_pipeline(input_folder, output):
    ...
    for i in [1]:
        ...
        values_cmd = f'python get_values.py -i {input_folder}/chr{i}_matrixeQTL_raw_rs1702294.tsv -z {output}/chr{i}/{zscore_file}_chr{i}'.split(' ')
    ...
    #for i in [7]:
        ...
        values_cmd = f'python tstat_to_pvalue.py -i {output}/chr{i}/{zscore_file}_chr{i} -p {output}/chr{i}/{pvalue_file}_chr{i}'.split(' ')
        #values = subprocess.Popen(values_cmd).wait()
    ...
    #for i in range(1, 23):
    for i in [1]:
    #for i in [9, 10, 12, 13, 14, 15, 16, 18, 20, 21, 22]:
        mzscores = f'{output}/chr{i}/{eqtl_file}_meanzscores_chr{i}.gz'
        evalues_file = f'{output}/chr{i}/{eqtl_file}_evalues_chr{i}.gz'
        evectors_file = f'{output}/chr{i}/{eqtl_file}_Q_chr{i}.gz'
        num_sim = 500000
        sim_file = f'{output}/chr{i}/{eqtl_file}_sim{num_sim}_cpma_chr{i}_converted'
        sim_mix_file = f'{output}/chr{i}/{eqtl_file}_sim{num_sim}_cpma-mix_chr{i}_converted'
